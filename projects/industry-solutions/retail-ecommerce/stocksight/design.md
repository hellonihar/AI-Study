...
Every technology choice in StockSight was evaluated against alternatives across six dimensions: operational complexity, team familiarity, scalability, cost, ecosystem fit, and production maturity. The table below covers all major layers of the stack.

| Layer | Chosen | Alternatives Considered | Rationale |
|---|---|---|---|
| **API Framework** | FastAPI + Uvicorn | **Django REST**: Mature ORM, admin panel, but heavier — adds 200ms+ to cold start, synchronous by default, fewer native async patterns. Complexity tax outweighs benefit for a service with no admin UI or complex relational queries. <br><br> **Flask**: Familiar but no async support, no automatic OpenAPI docs, manual validation. Would need Flask-Smorest + Connexion + gevent to approach FastAPI's ergonomics — inconsistent ecosystem. <br><br> **Node.js/Express**: Strong async I/O, but the team is Python-first. Adding Node.js to the stack splits CI/CD, monitoring, and on-call expertise across two runtimes. | FastAPI is Python-native (team expertise), natively async for SSE and streaming, auto-generates OpenAPI docs, and its Pydantic integration eliminates a separate schema layer. Cold start under 100ms — irrelevant for a long-running service. |
| **Frontend** | Next.js + Recharts | **FastAPI + Jinja2 + HTMX**: Single deployable, no Node.js build step, simpler ops. But inventory planners need interactive time-series charts — zoomable multi-horizon overlays, what-if sliders, real-time comparison views. HTMX round-trips make these painfully slow. Chart.js lacks the composable time-series primitives Recharts provides natively. <br><br> **Blazor/WebAssembly**: .NET dependency — no team expertise, adds a separate build toolchain. <br><br> **Streamlit**: Fast prototyping but no production-grade routing, auth integration, or chart customization. Suitable for internal demos, not a daily-driver dashboard. | Next.js provides SSR for initial load performance and Recharts for interactive charting. SWR handles cache-first data fetching — ideal for forecast data refreshed daily. The Node.js runtime is already standard in the organization's web team. |
| **Time-Series Database** | TimescaleDB (PostgreSQL 16) | **InfluxDB**: Purpose-built time-series DB with Flux query language. But it is a separate system to operate (backup, clustering, monitoring) and cannot join time-series with relational data (SKU master, stores). StockSight's forecast queries always join `forecasts` with `skus` and `stores` — TimescaleDB handles this in a single query. <br><br> **ClickHouse**: Blazing-fast columnar analytics for OLAP workloads. Overkill for StockSight's write volume (~50M rows/month). No native row-level updates — forecast corrections would require full partition rewrites. Joins are possible but not idiomatic (dictionaries vs. foreign keys). <br><br> **QuestDB**: Great ingestion performance but younger ecosystem, fewer client libraries, no managed cloud option. | TimescaleDB extends PostgreSQL with hypertables, continuous aggregates, native columnar compression, and automatic data retention — no new query language to learn, no separate cluster to operate. The team already knows PostgreSQL. Joins are first-class. |
| **Message Stream** | Kafka (POS ingestion) + RabbitMQ (Celery) | **Redis Pub/Sub**: Simple, fast, no persistent queueing. Messages are lost if no subscriber is listening — unacceptable for POS transaction ingestion where every sale must be recorded. No consumer group semantics for load-balanced processing. <br><br> **NATS**: Excellent performance and simple clustering but younger ecosystem — fewer client libraries, no native Kafka Connect ecosystem for integrating with existing Snowflake sinks. <br><br> **Single queue (RabbitMQ for everything)**: RabbitMQ can handle both use cases, but Kafka's partitioned log model is better for replaying POS data during model retraining (re-process 30 days of transactions). RabbitMQ's topic exchanges would require manual replay logic. | Kafka handles the high-throughput, replayable POS stream with exactly-once semantics. RabbitMQ handles Celery task dispatch with per-queue routing and TTL-based task expiry. Two systems, each purpose-built — Kafka for streaming, RabbitMQ for task orchestration. |
| **Object Store** | MinIO | **AWS S3**: Managed, zero ops. But the organization's cloud strategy is hybrid — some data (competitor pricing, scraped content) should not leave the network. MinIO provides an S3-compatible API on local storage with the same SDK interface. <br><br> **Azure Blob / GCS**: Cloud-locked — cannot run in on-premise or air-gapped deployments. MinIO runs anywhere Docker runs. <br><br> **Local filesystem**: No durability, no replication, no versioning. Model artifacts are irreplaceable — losing a fine-tuned checkpoint costs weeks of training time. | MinIO is S3-compatible at the API level (boto3 SDK works unchanged), runs as a single Docker container, supports versioning and replication, and works in any environment (dev laptop, on-prem, cloud). Zero lock-in — switch to S3 by changing the endpoint URL. |
| **Vector Store** | pgvector | **Qdrant**: Excellent filtering performance (full-text + vector pre-filtering), dedicated binary. But adds a separate Docker service with its own persistence, backup, and monitoring surface. At StockSight's scale (10K SKU embeddings, 384 dimensions), Qdrant's throughput advantage is irrelevant — pgvector queries complete in <5ms. <br><br> **Pinecone**: Managed SaaS — zero ops, excellent performance. But SKU embeddings are derived from sales data that should not leave the network per data governance policy. Also adds $200+/month for a task that runs once daily. <br><br> **Milvus**: Purpose-built vector DB with GPU acceleration. Significant ops complexity (etcd, Kafka, MinIO as dependencies in the Milvus cluster). Designed for billion-scale workloads — overkill for 10K SKU embeddings. | pgvector is an extension on the existing PostgreSQL — no new service to operate. Performance (<5ms for 10K vectors with HNSW index) is well within requirements for cold-start similarity search that runs once daily. |
| **Auth / Identity** | Keycloak | **Auth0**: SaaS — zero ops, excellent DX, social login. $0 starter tier, but costs scale with MAU. Data residency requirements may apply — the retailer operates in jurisdictions with data localization laws. Vendor lock-in: migrating off Auth0 later requires rebuilding all OIDC flows. <br><br> **Azure AD / Entra ID**: Already used for corporate email and Office 365. Good integration but license cost per external user (supply chain partners accessing the dashboard) adds overhead. No self-service registration for external planners. <br><br> **DIY with JWT + bcrypt**: Simple, no dependencies. But no MFA, no LDAP sync, no session management, no social login. Building these features from scratch adds 2-3 weeks of security-critical development. | Keycloak is self-hosted, open-source, OIDC-compliant, and supports MFA, LDAP sync, social login, and client-scoped roles out of the box. Runs as a single Docker container with PostgreSQL backing. Zero vendor lock-in — any OIDC provider (Auth0, Cognito, Azure AD) can replace it by changing the well-known URL. |
| **Feature Store** | Custom (Python library + TimescaleDB) | **Feast (Open Source)**: Purpose-built feature store with serving API, point-in-time joins, and online/offline store support. Already in pilot at the organization. But Feast lacks native support for time-series window features (rolling 28-day mean) — these would need custom transformation plugins or pre-computed materialized views. <br><br> **Tecton**: Managed feature platform — zero ops, built-in time-series transformations, feature serving with low latency. Cost-prohibitive at ~$15K/year for StockSight's scale. Vendor lock-in on feature definitions. <br><br> **dbt + Snowflake**: Existing analytics stack. Good for batch feature computation but no online serving (needed for real-time forecast API). Point-in-time correctness requires complex SQL window functions. | A custom feature store built on TimescaleDB continuous aggregates provides time-series features (rolling windows, lags) with native SQL, serving via the same FastAPI/PostgreSQL stack, and zero additional infrastructure. If the organization's Feast deployment matures to support time-series features, migrating is straightforward — the feature definitions are the same, only the serving layer changes. |
| **Model Serving** | ONNX Runtime | **TorchServe**: Native PyTorch serving with model versioning, REST/gRPC endpoints, and batching. Adds a Java dependency (the model server is Java-based) and requires GPU for practical latency. Team has no Java expertise. <br><br> **TF Serving**: Purpose-built for TensorFlow SavedModel format. StockSight's models are PyTorch-based (Lag-Llama) — exporting to TF SavedModel loses the ONNX optimization pass. <br><br> **BentoML**: Excellent DX for packaging models with custom pre/post-processing, but adds a gRPC + Yatai (model management) dependency. At StockSight's scale (<20ms per inference, 2 workers), BentoML's batching and auto-scaling features provide no benefit. | ONNX Runtime is a lightweight (~20MB) CPU inference engine that consumes the standard ONNX interchange format. Any framework (PyTorch, TensorFlow, JAX) can export to ONNX. Inference runs in-process within the Celery worker — no separate model server to deploy, scale, or monitor. |
| **Baseline Model** | Prophet (Meta) | **ARIMA (statsmodels)**: Classic statistical method, well-understood, no training required per SKU. But ARIMA cannot incorporate external regressors (weather, promotions, holidays) without manual pre-processing. Requires stationary data — retail sales have strong trend and seasonality that need manual differencing. <br><br> **Exponential Smoothing (ETS)**: Handles trend and seasonality natively, fast to fit. No external regressor support — cannot capture promotion lift or weather effects. <br><br> **Theta method**: Simple and robust for seasonal data but no regressor support and no uncertainty quantification (prediction intervals). | Prophet handles trend, seasonality, holidays, and external regressors in a single unified model. Fits quickly (15min per 1K SKUs), provides uncertainty intervals natively, and is robust to missing data and outliers common in retail sales data. The team already uses Prophet for marketing mix modeling. |
| **Observability** | Prometheus + Grafana + Loki + Tempo | **Datadog**: Excellent UX, single agent for metrics/traces/logs, APM auto-instrumentation. ~$15K/month for StockSight's scale (3 services, 10 containers) — 5× the infrastructure cost of the application itself. Data egress costs for shipping metrics to SaaS. <br><br> **New Relic**: Similar value proposition and cost structure to Datadog. Better APM, weaker metric aggregation. <br><br> **Elastic (ELK)**: Industry-standard log aggregation and search. But Elastic APM for tracing is less mature than Tempo, and the stack (Elasticsearch, Kibana, APM Server, Fleet) requires significant ops investment — 4 services vs. Prometheus/Grafana's 2. | Prometheus + Grafana + Loki + Tempo is the standard open-source observability stack. Prometheus scrapes metrics, Loki ingests structured logs, Tempo traces distributed requests, Grafana ties them together. All four run as Docker containers with no external dependencies, no licensing cost, and no data egress. The team already operates this stack for 5 other services. |
| **CI/CD** | GitHub Actions + ArgoCD + Helm | **GitLab CI**: Equivalent feature set (container registry, CI runners, multi-stage pipelines). Not chosen because the code is hosted on GitHub — GitLab CI would require mirroring or a separate GitLab instance. <br><br> **Jenkins**: Extensible, mature, vast plugin ecosystem. Adds a Java dependency and a dedicated master/agent cluster to maintain. Pipeline-as-code is second-class (Jenkinsfile vs. native YAML in GitHub Actions). <br><br> **CircleCI**: Fast CI, excellent parallelism. No built-in container registry — would need Docker Hub or ECR as a separate service. Cost scales with concurrency — expensive at StockSight's build frequency (multiple daily commits). | GitHub Actions is co-located with the code repository, requires no additional service, and provides integrated container registry (GHCR). ArgoCD provides GitOps deployment to K3s with automated sync and drift detection. Helm packages the Kubernetes manifests for reproducible deployments. |
| **Infrastructure** | K3s (dev) / EKS (prod) | **Docker Swarm**: Simpler than Kubernetes — native Docker CLI, no etcd, no Ingress controller setup. But Swarm mode is in maintenance mode (Docker shifted focus to Kubernetes), has no ecosystem of operators/CRDs, and no built-in secrets management or RBAC. <br><br> **GKE / AKS**: Managed Kubernetes — excellent control planes, integrated with cloud IAM. Not chosen over EKS because the organization's primary cloud provider is AWS. <br><br> **Nomad + Consul**: HashiCorp's simpler orchestrator with service mesh. Excellent for batch jobs (model training) but weaker for long-running services (no native Ingress, no HPA, no built-in service discovery without Consul Connect). | K3s provides a lightweight, CNCF-certified Kubernetes distribution for dev and staging — single binary, SQLite-backed (no etcd), runs on a single VM. EKS provides the production-grade managed control plane with AWS IAM integration, ALB Ingress, and EBS/EFS CSI drivers. The Helm charts are identical between environments — only the kubeconfig changes. |
